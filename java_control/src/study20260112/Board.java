...
public class Board {
	...
	public void hitUp(String user) {
		// writer 는 해당 글의 작성자, user 는 글 열람한 사람
		if (!user.equals(writer)) hit++;
	}
	
	
	// 게시글의 제목 수정
	// 게시글의 작성자가 아니면 수정할 수 없다.
//	public void editTitle(String title, String user) {
//		if (user.equals(writer)) this.title = title;
 ...
 *		}
 *
 *			*default 는 웬만하면 사용하지 않는 것이 좋다
 *			**private 는 클래스 내부에서만 사용할 메서드, 굳이 다른 클래스에서 사용할 필요가 없는 메서드에 사용
